...
let sql = require('./utils/sql') //insert func
...
function work (arr){
  //循环数组，队列工作
  for(let i in arr){
    queue.push(getDetail,[arr[i]]).then((result) => console.log(`当前队列完成${result}`)).catch((e)=>{
  ...
  return new Promise((resolve, reject) => {
    getCar(model).then((res) => {
      if(!res.result){
        ...
        return getCarDetailPromise(id, model)
      }
    }).then((arr) => {
      return sql.insert(arr)
    }).then((result)=>{
      resolve(result)
    })
    .catch((arr) => {
      console.log(`getCarErr2:` + arr)
      if(typeof arr == Array){
        return sql.insert(arr)
      }
    })
...
function getCarDetailPromise(id, model){
  return new Promise((resolve, reject) => {
    getCarDetail(id).then((res) => {
      ...
      if(fadongjiData.length>4){
        fadongjiData.slice(0,3)
      }
      let data = mainDetailData.concat(dipanDetailData, fadongjiData)
      if(data.length!=35){
        console.log(mainDetailData,dipanDetailData,fadongjiData)
        data.concat(new Array(33))
        console.log(data)
      }
      resolve(data)
    }).catch((err) => {
...
if(!this.isStart() && this.getLength() >= 10){ //当添加了2个项后,运行队列
